...
class AbsensiController extends Controller
{
    ...
    public function create(Request $request){
        ...
        for($i=0;$i<count($request->mahasiswa);$i++){
            ...
            $check = Absensi::where(['mahasiswa_id' => $request->mahasiswa[$i],'kelas_id' => $request->kelas, 'tanggal' => Carbon::now('Asia/Jakarta')->format('Y-m-d')])->get();
            if(count($check)==0 && $request->status[$i] != "Hadir"){
                $absen = new Absensi;
                $absen->mahasiswa_id = $request->mahasiswa[$i];
