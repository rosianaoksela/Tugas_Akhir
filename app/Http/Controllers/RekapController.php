...
class RekapController extends Controller
{
    ...
    public function pdf(Request $request){
        ...
        foreach($resource->mahasiswa as $index => $res){
            $hadir=Absensi::where(['mahasiswa_id'=>$res->id_mahasiswa])->where('status','Hadir');
            $alfa=Absensi::where(['mahasiswa_id'=>$res->id_mahasiswa])->where('status','Alfa');
            $izin=Absensi::where(['mahasiswa_id'=>$res->id_mahasiswa])->where('status','Izin');
            $sakit=Absensi::where(['mahasiswa_id'=>$res->id_mahasiswa])->where('status','Sakit');
        ...
        $pdf->loadHTML(view("layouts.pribadipdf",['resource'=>$resource, 'tanggal'=>$tanggal, 'hadir'=>$hadir,'alfa'=>$alfa,'izin'=>$izin,'sakit'=>$sakit]));
        return $pdf->stream();
    }
