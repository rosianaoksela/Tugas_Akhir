            ...
            <div class="panel panel-primary">
                ...
                <div class="panel-body">
                    <div class="table-responsive">
                        <div class="table-responsive">
                            <table id="absensi" class="table table-bordred table-striped">
                                ...
                                <tbody>
                                    @foreach($resource->mahasiswa as $index => $mahasiswa)
                                    <tr>
                                        <td><input type="checkbox" class="checkthis" /></td>
                                        <td class="text-center">{{ $index+1 }}</td>
                                        <td>{{ $mahasiswa->nim}}</td>
                                        <td>{{ $mahasiswa->nama}}</td>
                                        <td class="text-center">@if($mahasiswa->jenis_kelamin=="L"){{ "Laki-Laki" }}@else{{ "Perempuan" }}@endif</td>
                                        <td class="text-center"><p data-placement="top" data-toggle="tooltip" title="Delete"><button data-aksi="MahasiswaKelas" data-id="{{$mahasiswa->pivot->id_mahasiswa_kelas}}" class="delete-button btn btn-danger btn-xs" data-title="Delete" data-toggle="modal" data-target="#delete"><span class="glyphicon glyphicon-trash"></span></button></p></td>
                                    </tr>
                                    @endforeach
                                </tbody>
                            </table>
                    ...
                    <div class="panel-footer">
                        <form action="{{ url('/mahasiswakelas/'.$resource->id_kelas) }}" method="GET"> 
